...
REMOTE_STAGE="/cache/wear5_delta_restore.bin"
MAX_STAGE_CHUNKS=2
...
for ROW in "${RANGES[@]}"; do
  ...
  TOTAL_GROUPS=$((TOTAL_GROUPS + (COUNT + MAX_STAGE_CHUNKS - 1) / MAX_STAGE_CHUNKS))
done

echo "STAGING_METHOD=/cache_verified_then_local_dd"
echo "MAX_STAGE_BYTES=$((MAX_STAGE_CHUNKS*BS))"
echo "TRANSFER_GROUPS=$TOTAL_GROUPS"

GROUP=0
for ROW in "${RANGES[@]}"; do
  IFS=$'\t' read -r RUN_START RUN_COUNT <<<"$ROW"
  OFFSET=0
  while [ "$OFFSET" -lt "$RUN_COUNT" ]; do
    COUNT=$MAX_STAGE_CHUNKS
    REM=$((RUN_COUNT-OFFSET))
    [ "$REM" -lt "$COUNT" ] && COUNT=$REM
    START=$((RUN_START+OFFSET))
    BYTES=$((COUNT*BS))
    GROUP=$((GROUP+1))

    LOCAL_STAGE="$TMP/stage.bin"
    dd if="$OLD" of="$LOCAL_STAGE" bs="$BS" skip="$START" count="$COUNT" status=none
    LOCAL_HASH="$(sha256sum "$LOCAL_STAGE" | awk '{print $1}')"

    echo "DELTA_GROUP=$GROUP/$TOTAL_GROUPS START_CHUNK=$START CHUNKS=$COUNT BYTES=$BYTES"

    "${ADB[@]}" push "$LOCAL_STAGE" "$REMOTE_STAGE" </dev/null >/dev/null       || die "push_group_${GROUP}"

    STAGE_HASH="$("${ADB[@]}" shell "sha256sum '$REMOTE_STAGE' 2>/dev/null" </dev/null | tr -d '\r' | awk '{print $1}' | tail -1)"
    [ "$STAGE_HASH" = "$LOCAL_HASH" ]       || die "stage_hash_group_${GROUP}_expected_${LOCAL_HASH}_got_${STAGE_HASH}"

    "${ADB[@]}" shell "dd if='$REMOTE_STAGE' of='$SUPER_DEV' bs=$BS seek=$START count=$COUNT conv=notrunc,fsync 2>/dev/null" </dev/null       || die "write_group_${GROUP}"
    ...
    [ "$REMOTE_HASH" = "$LOCAL_HASH" ]       || die "verify_group_${GROUP}_expected_${LOCAL_HASH}_got_${REMOTE_HASH}"

    echo "DELTA_GROUP_VERIFY=PASS"
    OFFSET=$((OFFSET+COUNT))
  done
done

"${ADB[@]}" shell "rm -f '$REMOTE_STAGE'; sync" </dev/null >/dev/null 2>&1 || die "sync_fallito"

echo "VERIFYING=full_super_sha256"
REMOTE_FULL="$("${ADB[@]}" shell "sha256sum '$SUPER_DEV' 2>/dev/null" </dev/null | tr -d '\r' | awk '{print $1}' | tail -1)"
[ "$REMOTE_FULL" = "$OLD_EXPECTED" ]   || die "full_super_hash_expected_${OLD_EXPECTED}_got_${REMOTE_FULL}"
echo "FULL_SUPER_VERIFY=PASS"
echo "REMOTE_FULL_SHA256=$REMOTE_FULL"

echo "DELTA_RESTORE=PASS"
echo "RESTORED_TO=CANDIDATE1"
echo "EXPECTED_FULL_SUPER_SHA256=$OLD_EXPECTED"
echo "FULL_HASH_VERIFIED=YES"
echo "RECOVERY_TOUCHED=NO"
